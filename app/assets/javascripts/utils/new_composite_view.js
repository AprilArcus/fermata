Backbone.CompositeView = Backbone.View.extend({
  addSubview: function (subview, selector) {
    this.subviews(selector).push(subview);
    // Try to attach the subview. Render it as a convenience.
    this.attachSubview(subview.render(), selector);
  },

  attachSubview: function (subview, selector) {
    if (selector) {
  ...
  attachSubviews: function () {
    ...
    _(this.subviews()).each(function (subviews, selector) {
      if (selector) view.$(selector).empty();
      _(subviews).each(function (subview) {
        view.attachSubview(subview, selector);
      });
    });
  },
  ...
  removeSubview: function (subview, selector) {
    subview.remove();
